...
export async function POST(request: Request) {
    ...
    try {
        ...
        // BUT if we were passed pre-created IDs, we explicitly bypass active job check since they were generated by the onboard click
        if (!body.job_id) {
        ...
        if (body.decision_id && body.job_id) {
            decisionId = body.decision_id;
            jobId = body.job_id;
            console.log('[strategies/generate] Using pre-created decision and job IDs:', decisionId, jobId);
        } else {
            // ── Create decision row ──
            ...
            console.log('[strategies/generate] Decision created:', decisionId);
...
Generate 3 strategies ranked from best fit (rank 1) to least fit (rank 3). Each must have all fields shown above.`;
            ...
            try {
                ...
                console.log(`[strategies/generate] Gemini responded in ${latencyMs}ms (${rawText.length} chars)`);
                 ...
                 logUsageAndCost(supabase, {
                     ...
                     isExempt: !!body.is_exempt || !!body.job_id,
        ...
        console.log('[strategies/generate] ✅ DONE! Job:', jobId, 'Decision:', decisionId);
