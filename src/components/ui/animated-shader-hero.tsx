...
// Reusable Hero Component (Using native CSS styling and CSS variables for maximum isolation)
...
void main(void) {
	...
	uv*=1.-.2*(sin(T*.08)*.5+.5);
	for (float i=1.; i<8.; i++) {
		uv+=.12*cos(i*vec2(.1+.01*i, .85)+i*i+T*.25+.12*uv.x);
		vec2 p=uv;
		float d=length(p);
		col+=.00125/d*(cos(sin(i)*vec3(1.0,0.85,0.25))+1.0);
		float b=noise(i+p+bg*1.731);
		col+=.002*b/length(max(p,vec2(b*p.x*.02,p.y)));
		col=mix(col,vec3(bg*.15,bg*.09,bg*.025),d);
	}
	O=vec4(col,1);
}`;
